...
export type NewsArticle = {
  ...
  title: string;
  dek: string;
  /** Optional hero/thumbnail photo. Falls back to gradient when omitted. */
  image?: {
    src: string;
    alt: string;
  };
  /** CSS gradient classes for thumbnail when no image is set */
...
export function getNewsArticle(
  ...
  return newsArticles.find((article) => {
    const segments = newsArticleSegments(article);
    return (
      segments.year === year &&
