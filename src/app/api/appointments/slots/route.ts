...
import { getOpenSlotsForDate } from "@/lib/appointments/slots";
...
export async function GET(request: Request) {
  ...
  const slots = getOpenSlotsForDate(config, appointments, date, {
    serviceId,
    oneWayKm,
  }).map((slot) => slot.slice(11, 16));

  return NextResponse.json({ slots, serviceId, oneWayKm: oneWayKm ?? null });
}
